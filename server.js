...
// Debug log for Cloudinary env (optional)
...
// Multer storage config that stores images in /server/images and videos in /server/video
...
// ---------- Raw body endpoint for payment webhook ----------
// Keep this BEFORE express.json so raw body is preserved.
app.post(
...
app.post("/api/tracking/webhook/aftership", express.json(), async (req, res) => {
  try {
    ...
    const trackingNumber =
      payload?.data?.tracking?.tracking_number || payload?.tracking_number;

    if (trackingNumber) {
      const Tracking = (await import("./models/Tracking.js")).default;
      const orderTracking = await Tracking.findOne({ trackingNumber }).sort({
        createdAt: -1,
      });
      ...
      if (orderTracking) {
        ...
        orderTracking.status =
          payload?.data?.tracking?.tag || orderTracking.status;
        await orderTracking.save();

        // If delivered, update order status
        if (orderTracking.status === "Delivered") {
          const Order = (await import("./models/Order.js")).default;
          const order = await Order.findById(orderTracking.orderId).populate(
            "userId",
            "name email"
          );
          ...
          if (order) {
            ...
            await order.save();
            // Optional: send rating email via sendEmail utility
          }
        }
      }
    }
  ...
  (req, res) => {
    try {
      ...
      const imageUrls = (files.images || []).map(
        (f) => `${protocol}://${host}/images/${f.filename}`
      );
      const videoUrls = (files.video || []).map(
        (f) => `${protocol}://${host}/video/${f.filename}`
      );
      ...
      return res.status(201).json({
        ...
        filenames: {
          ...
          video:
            (files.video && files.video[0] && files.video[0].filename) || "",
        },
      });
    } catch (err) {
